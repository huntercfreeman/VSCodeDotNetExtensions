<script lang="ts">
    ...
    function addNugetPackageReference() {
        let messageUpdateAddNugetPackageReference =
            new MessageUpdateAddNugetPackageReference(
                selectedProjectFile,
                nugetPackageModel,
...
<div>
    <div>
        ...
        <span
            >{nugetPackageModel.title} ({nugetPackageModel.totalDownloads.toLocaleString()}:
            downloads)</span
        >
    </div>
    {#if nugetPackageIsExpanded}
        <div class="dni_tree-view-children">
            <div>
                <SelectNugetPackageVersionForm
                    bind:selectedVersionModel
                    versionModels={nugetPackageModel.versions}
                />
            </div>
            <div>
                Chosen Version: {selectedVersionModel?.version ??
                    "undefined"}&nbsp;
                {#if selectedVersionModel}
                    ({selectedVersionModel.downloads.toLocaleString()}:
                    downloads)
                {/if}
            </div>

            <button on:click={addNugetPackageReference}>
                Add to {selectedProjectFile.absoluteFilePath
                    .filenameWithExtension}
            </button>

            <div>
                <ExpansionChevron bind:isExpanded={descriptionIsExpanded} />
            ...
            <div>
                <ExpansionChevron bind:isExpanded={summaryIsExpanded} />
                <span>summary:</span>
            ...
            {#if metaDataIsExpanded}
                <div class="dni_tree-view-children">
                    ...
                    <div>
                        totalDownloads: {nugetPackageModel.totalDownloads}
                    </div>
